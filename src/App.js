...
const bootstrapDatabase = () => {
  return flureeFetch('/new-db', { 'db/id': 'credentials/demo' })
    .then(res => delay(2500))
...
class App extends React.Component {
  ...
  componentDidMount() {
    ...
    flureeFetch('/query', query)
      .then(res => {
        this.setState({
          orgName: res['verificationOrg/name'],
          applicants: res.applicants
        });
      })
      .catch(err => {
        if (/not found/.test(err.message)) {
          bootstrapDatabase().then(res => window.location.reload());
        }
      });
  }
    ...
    flureeFetch('/transact', transaction)
      .then(res => {
        const updatedApplicant = {
          ...this.state.applicants.find(a => a._id === _id),
          'user/verificationStatus': status
        };
        const otherApplicants = this.state.applicants.filter(
          a => a._id !== _id
        );
        this.setState(prevState => ({
          ...prevState,
          applicants: [...otherApplicants, updatedApplicant]
        }));
      })
  ...
  fetchHashHist = async hash => {
    ...
    flureeFetch('/history', query)
      .then(res => {
        const flakes = res.reduce(
          (memo, current) => memo.concat(current.flakes),
          []
        );
        if (!flakes[0]) {
          return;
        ...
        flakes.forEach((flake, i) => {
          multiQuery[i] = {
            select: ['?userName', '?blockNo', '?blockInstant'],
            where: [
        ...
        flureeFetch('/multi-query', multiQuery)
          .then(res => {
            ...
            Object.values(res).forEach((authBlockArray, i) => {
              data.push(flakes[i].concat(authBlockArray[0]));
            });
  ...
  fetchCertHist = async certId => {
    ...
    return flureeFetch('/history', query)
      .then(res => {
        const flakes = res.reduce(
          (prev, current) => prev.concat(current.flakes),
          []
        );
        const assertions = flakes.filter(flake => flake[4]);
        let multiQuery = {};
        ...
        return flureeFetch('/multi-query', multiQuery)
          .then(res => {
            ...
            const certificateData = data.map(
              flake => new CertificateHistory(flake)
            );
  ...
  checkHash = applicant => {
    ...
    const query = {
      selectOne: ['*', { 'certificate/university': ['name'] }],
  ...
  render() {
    ...
    switch (this.state.tabIndex) {
      case 1:
        table = (
          <TranscriptTable
            activeCert={this.state.certificateID}
...
class ApplicantTable extends React.Component {
  ...
  onChange = async (event, _id) => {
    ...
    reader.onload = event => {
      ...
      const updatedApplicant = {
        ...this.props.applicants.find(a => a._id === _id),
        hash: encrypted
      };
        ...
        <Table>
          ...
          <TableBody>
            {data
              ...
              .map(n => {
                const isActive =
                  updatedApplicant && n._id === updatedApplicant._id;
                    ...
                    <TableCell>
                      {isActive ? (
                        ...
                        <input
                          id='upload'
                          type='file'
                          onChange={e => this.onChange(e, n._id)}
                        />
                      )}
                    </TableCell>
                    <TableCell>
  ...
  render() {
    ...
    if (data[0]) {
      ...
      setTimeout(
        () =>
          window.alert(
            'The last document uploaded has never been associated with a transcript in the blockchain.'
          ),
        200
      );
    } else {
      setTimeout(
        () => window.alert('Please upload a transcript for verification.'),
        200
      );
    }
    return (
  ...
  render() {
    ...
    if (data[0]) {
      ...
      setTimeout(
        () =>
          window.alert(
            'The last document uploaded did not match any certificate active in the database.'
          ),
        200
      );
    } else {
